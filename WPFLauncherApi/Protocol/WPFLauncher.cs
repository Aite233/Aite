...
public static class WPFLauncher {
    ...
    public static async Task CreateCharacterAsync(string gameId, string roleName)
    {
        ...
        {
            Log.Error("创建游戏角色 {0} 失败 {1}", roleName, e.Message);
            throw new Exception($"创建游戏角色 {roleName} 失败: {e.Message}", e);
        }
    }
    ...
    */
    ...
    public static async Task CreateCharacterRentalAsync(string serverId, string roleName)
    {
        ...
        {
            Log.Error("创建租赁服游戏角色 {0} 失败 {1}", roleName, e.Message);
            throw new Exception($"创建租赁服游戏角色 {roleName} 失败: {e.Message}", e);
        }
    }
