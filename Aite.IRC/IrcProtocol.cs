...
{
    public const string Host = "103.40.13.4";
    public const int Port = 52188;

    public static string Register(string roleId, string clientTag = "")
        => string.IsNullOrEmpty(clientTag)
            ? $"REGISTER {roleId}"
            : $"REGISTER {roleId} {clientTag}";

    public static string Get(string roleId)
        => $"GET {roleId}";

    public static string Chat(string roleId, string msg) 
        => $"CHAT {roleId} {msg}";
