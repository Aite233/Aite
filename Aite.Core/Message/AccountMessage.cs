...
public static class AccountMessage {
    ...
    public static EntityAccount[] GetLoginAccountList()
    {
        ...
        var loginAccounts = accountList.Where(account => !string.IsNullOrEmpty(account.UserId)).ToArray();
        Log.Information("GetLoginAccountList: {Count} accounts found", loginAccounts.Length);
        foreach (var account in loginAccounts)
        {
            Log.Information("Account: {Account}, UserId: {UserId}, Type: {Type}", account.Account, account.UserId, account.Type);
        }
    ...
    private static (EntityAccount[], string) GetAccountList1(bool defaultLogin = true)
    {
        ...
        foreach (var item in entity) {
            var flag = InfoManager.GameAccountList.Any(gameAccount => 
                (item.Type == "cookie" && gameAccount.Type == "cookie" && item.Password == gameAccount.Password) ||
                (item.Type != "cookie" && gameAccount.Type != "cookie" && item.Account == gameAccount.Account && item.Password == gameAccount.Password)
            );
            if (!flag) {
                item.UserId = null;
                item.Token = null;
